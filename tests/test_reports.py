...
def test_spending_by_category_with_cashback(sample_transactions, category,
                                             expected):
...
@pytest.mark.parametrize(
    "amount,expected", [(0.00, 2098.94), (100.00, 2098.94), (-100.00, 2198.94)]
)
def test_spending_by_category_zero_amount(sample_transactions, amount, expected):
    zero_transaction = pd.DataFrame([{"Дата операции": "2023-10-30", "Сумма операции": amount, "Кешбэк": 0.00, "Категория": "Супермаркеты", "Описание": "Тестовая транзакция"}])
...
def test_spending_by_category_cashback_variations(cashback, expected):
    transactions = pd.DataFrame([{"Дата операции": "2023-10-01", "Сумма операции": -100.00, "Кешбэк": cashback, "Категория": "Тестовая категория", "Описание": "Тестовая транзакция"}])
...
def test_spending_by_weekday(sample_transactions):
    ...
    result = json.loads(spending_by_weekday(sample_transactions, date="2023-10-15"))
    assert isinstance(result, dict)
    assert all(
        day in result
        for day in ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday']
    )
    assert all(isinstance(result[day]['mean'], float) for day in result)
    assert all(isinstance(result[day]['count'], int) for day in result)
    assert all(result[day]['mean'] >= 0 for day in result)
    assert all(result[day]['count'] >= 0 for day in result)
...
def test_spending_by_weekday_with_income(sample_transactions):
    ...
    df = pd.concat([sample_transactions, income], ignore_index=True)
    result = spending_by_weekday(df, date="2023-10-15")
    result_dict = json.loads(result)  # Исправлено: result_dict объявлен здесь
    assert isinstance(result_dict, dict)
    assert all(
...
def test_spending_by_workday_empty_df():
    ...
    assert result == {"Рабочий день": {"mean": 0.0}, "Выходной день": {"mean": 0.0}}
...
def test_spending_by_category_with_nan_values():
    ...
    nan_transactions = pd.DataFrame([
        {"Дата операции": "2023-10-01", "Сумма операции": -100.00, "Кешбэк": pd.NA, "Категория": "Тестовая категория", "Описание": "Тестовая транзакция"},
...
def test_spending_by_weekday_different_date_formats(sample_transactions):
    ...
    for date in ["2023-10-15", "15.10.2023", "2023/10/15"]:
        result_dict = json.loads(spending_by_weekday(sample_transactions, date=date))
        assert isinstance(result_dict, dict)
        assert all(day in result_dict for day in [
...
def test_spending_by_category_large_amounts():
    ...
    assert spending_by_category(large_transactions, "Крупные покупки")["total"] == 1000000.00
...
def test_spending_by_category_small_amounts():
    ...
    assert spending_by_category(small_transactions, "Мелкие покупки")["total"] == 0.01
...
def test_spending_by_category_special_characters():
    ...
    assert spending_by_category(special_transactions, "Тест@#$%^&*()")["total"] == 100.00
...
def test_spending_by_category_unicode_characters():
    ...
    assert spending_by_category(unicode_transactions, "Тест🌍🌎🌏")["total"] == 100.00
...
def test_spending_by_category_case_sensitivity():
    ...
    assert spending_by_category(case_transactions, "Тестовая Категория")["total"] == 100.00


    # Должно быть 0, так как регистр важен
    assert spending_by_category(case_transactions, "тестовая категория")["total"] == 0.0
...
def test_spending_by_category_with_none_values():
    ...
    assert spending_by_category(none_transactions, "Тестовая категория")["total"] == 100.00
...
def test_spending_by_category_with_inf_values():
    ...
    assert spending_by_category(inf_transactions, "Тестовая категория")["total"] == 0.0
...
def test_spending_by_category_with_nan_category():  # Исправлено название теста
    ...
    assert spending_by_category(nan_category_transactions, pd.NA)["total"] == 0.0
...
def test_spending_by_category_with_very_large_numbers():  # Название теста
    ...
    large_number_transactions = pd.DataFrame([{
        "Дата операции": "2023-10-01", "Сумма операции": -1e15, "Кешбэк": 0.00, "Категория": "Тестовая категория", "Описание": "Тестовая транзакция"}])
...
def test_spending_by_category_with_very_small_numbers():  # Название теста
    ...
    small_number_transactions = pd.DataFrame([{
        "Дата операции": "2023-10-01", "Сумма операции": -1e-15, "Кешбэк": 0.00, "Категория": "Тестовая категория", "Описание": "Тестовая транзакция"}])
    small_number_transactions['Дата операции'] = pd.to_datetime(small_number_transactions['Дата операции'])

    assert spending_by_category(small_number_transactions, "Тестовая категория")["total"] == 1e-15


def test_spending_by_category_with_mixed_date_formats():  # Название теста
    """Тест на разные даты"""
    mixed_dates_transactions = pd.DataFrame([{
        "Дата операции": "2023-10-01", "Сумма операции": -100.00, "Кешбэк": 0.00, "Категория": "Тестовая категория", "Описание": "Тестовая транзакция"}, {
        "Дата операции": "2023-10-02", "Сумма операции": -200.00, "Кешбэк": 0.00, "Категория": "Тестовая категория", "Описание": "Тестовая транзакция"}])
...
def test_spending_by_category_with_duplicate_categories():  # Название теста
    ...
    duplicate_categories_transactions = pd.DataFrame([{
        "Дата операции": "2023-10-01", "Сумма операции": -100.00, "Кешбэк": 0.00, "Категория": "Тестовая категория", "Описание": "Первая транзакция"}, {
        "Дата операции": "2023-10-02", "Сумма операции": -200.00, "Кешбэк": 0.00, "Категория": "Тестовая категория", "Описание": "Вторая транзакция"}])

    duplicate_categories_transactions['Дата операции'] = pd.to_datetime(duplicate_categories_transactions['Дата операции'])

    assert spending_by_category(duplicate_categories_transactions, 
    "Тестовая категория")["total"] == 300.00
...
def test_spending_by_category_with_mixed_amount_types():  # Название теста
    ...
    assert spending_by_category(mixed_amount_transactions, "Тестовая категория")
    ["total"] == 300.00
