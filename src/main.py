...
def main():
    ...
    # Инвесткопилка
    invest_result = investment_bank("2023-10", transactions, 50)
    print(f"\nИнвесткопилка: {invest_result}")
    ...
    phone_search_result = search_phone_numbers(transactions)  # type: ignore
    print(f"\nПоиск по телефонным номерам: {phone_search_result}")
    ...
    print(
        json.dumps(
            physical_transfer_search_result, ensure_ascii=False, indent=4
            )
        )
    ...
    weekday_rep = spending_by_weekday(df, date="2023-10-15")
    print("\nОтчет по дням недели:")
    print(json.dumps(weekday_rep, ensure_ascii=False, indent=4))
